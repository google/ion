...
// TODO(bug): For now we use a demo-specific utility to transform
...
static ion::gfxutils::ShaderSourceComposerPtr CreateShaderSourceComposer(
    std::string name, ion::gfx::GraphicsManager::GlApi api, GLuint glversion,
    bool is_fragment_shader) {
  ion::gfxutils::FilterComposer::StringFilter filter =
      std::bind(&RewriteShader, std::placeholders::_1, api, glversion,
                is_fragment_shader);
...
static void InitializePositionsBuffer(ion::gfx::BufferObjectPtr buffer,
...
static void InitializePropertiesBuffer(ion::gfx::BufferObjectPtr buffer,
                                       size_t particle_count) {
...
void IonPhysicsDemo::BuildVerletRoot() {
  ...
  ion::gfx::GraphicsManager::GlApi glapi = gm->GetGlApiStandard();
  unsigned int glversion = gm->GetGlVersion();
  ion::gfx::ShaderProgramPtr shader = shader_manager->CreateShaderProgram(
      "verlet", reg,
      CreateShaderSourceComposer("verlet.vp", glapi, glversion, false),
      CreateShaderSourceComposer("verlet.fp", glapi, glversion, true));
...
void IonPhysicsDemo::BuildRenderRoot() {
  ...
  ion::gfx::ShaderProgramPtr shader = shader_manager->CreateShaderProgram(
      "draw", reg,
      CreateShaderSourceComposer("draw.vp", glapi, glversion, false),
      CreateShaderSourceComposer("draw.fp", glapi, glversion, true));
  node->SetShaderProgram(shader);
...
DemoBase* CreateDemo(int w, int h) { return new IonPhysicsDemo(w, h); }
