...
// Collection of per-Visual call traces for a particular GraphicsManager.
// Stores an indentation depth and output stream for each active Visual.
...
// custom streams.  In addition to the per-visual string streams, calls
 ...
 public:
  ...
  class Proxy {
   public:
    ...
    ~Proxy() {
      if (active_)
        tracing_stream_->Append(portgfx::Visual::GetCurrentId(),
                                output_stream_.str());
    }
    Proxy(Proxy&& other)
  ...
  // In addition to the per-Visual string streams, clients can provide
  ...
  // Clear the stream for every Visual (does not reset the indentation levels).
  void Clear();

  // Fetch the call trace log for a particular Visual.
  std::string String(intptr_t visual_id) const;

  // Fetch the call trace log for the current Visual.
  std::string String() const;

  // Get a list of ids for all Visuals that made GL calls during the trace.
  std::vector<intptr_t> Keys() const;

  // Enable tracing from all Visuals.
  void StartTracing();

  // Disable tracing from all Visuals.
  void StopTracing();

  // Check if tracing is enabled.
  bool IsTracing() const;

  // Send output to the INFO log when tracing, optionally for all visuals.
  void EnableLogging(intptr_t visual_id = 0LL);

  // Stop sending output to the INFO log, optionally for all visuals.
  void DisableLogging(intptr_t visual_id = 0LL);

  bool IsLogging() const;

  // Called within Ion to append a string to the stream associated with
  // the given Visual.
  void Append(intptr_t visual_id, const std::string& s);
  ...
  void EnterScope(intptr_t visual_id, const std::string& marker);

  // Called within Ion to decrease the indentation level.
  void ExitScope(intptr_t visual_id);

  // Get the number of scopes that have been entered but not exited.
  int Depth(intptr_t visual_id) const;
...
inline void TracingStream::Append(intptr_t visual_id, const std::string& s) {}
...
inline std::string TracingStream::String(intptr_t visual_id) const {
  return "";
}
...
inline void TracingStream::EnableLogging(intptr_t visual_id) {}
inline void TracingStream::DisableLogging(intptr_t visual_id) {}
inline bool TracingStream::IsLogging() const { return false; }
inline void TracingStream::EnterScope(intptr_t visual_id,
                                      const std::string& marker) {}
inline void TracingStream::ExitScope(intptr_t visual_id) {}
inline int TracingStream::Depth(intptr_t visual_id) const { return 0; }
