...
static std::atomic<int>& GetGlobalResourceCount() {
  ION_DECLARE_SAFE_STATIC_POINTER(std::atomic<int>, resource_count);
 ...
 public:
  ...
  class Resource : public ResourceBase, public Allocatable {
   ...
   protected:
    explicit Resource(ResourceManager* rm, const ResourceHolder* holder,
        ...
        : ResourceBase(holder, key),
          ...
          unbind_ref_count_(0),
          released_(false) {
      CHECK(resource_manager_);
      ++GetGlobalResourceCount();
    }
    ...
    void SetReleased() { DCHECK(!released_); released_ = true; }
    ...
    // Marks the resource for deferred Unbind() in all ResourceBinders whose
    // ResourceManager is the same as the Resource's. Unbind is deferred until
    // the next GetOrCreateInternalResourceBinder to ensure thread-safety.
    void MarkForUnbindAll();

    // Each deferred Unbind() request increments a ref count so that the
    // Resource is not deleted until it is unbound in all ResourceBinders.
    void IncrementUnbindRefCount() { ++unbind_ref_count_; }
    void DecrementUnbindRefCount() { --unbind_ref_count_; }
    int GetUnbindRefCount() const { return unbind_ref_count_; }

    // Deletes this and returns true if IsReleased() is true and
    // GetUnbindRefCount() is 0.
    bool DeleteIfReleasedAndUnbound();
   ...
   private:
    ...
    std::atomic<int> unbind_ref_count_;
    bool released_;  // Set to true by Release().
  ...
  void MarkForRelease(Resource* resource) {
    resource->MarkForUnbindAll();
    std::lock_guard<std::mutex> locker(release_mutex_);
    DCHECK(resource);
    resources_to_release_.push_back(resource);
    // Remove from resources_ since resource is now invalid, i.e., has null
    // GetHolder().
  ...
  void RemoveResource(Resource* resource) {
    ...
    if (num_resources > 1U) {
      ...
      // If RemoveResource is called more than once, the resource will have
  ...
  void ReleaseTypedResources(ResourceType type) {
    // Copy ResourceVector into local storage so we don't keep ResourceAccessor
    // mutex which can result in deadlock with OnDestroyed()->MarkForRelease()->
    // RemoveResource().
    ResourceVector resources =
        ResourceAccessor(resources_[type]).GetResources();
  ...
  void ProcessReleases(bool can_make_gl_calls);
  void ProcessReleases() {
    ProcessReleases(AreResourcesAccessible());
  }
  ...
  void DestroyOrAbandonAllResources(bool force_abandon) {
    ...
    ProcessReleases(can_make_gl_calls);
    ...
    for (int i = 0; i < kNumResourceTypes; ++i) {
      ...
      for (Resource* resource : resources) {
        if (resource->IsValid()) {
          // Unbind resource from all ResourceBinders.
          for (ResourceBinderMap::iterator it = binders.begin();
               it != binders.end();
               ++it) {
            ResourceBinder* rb = it->second.get();
            // Note that the following Unbind() is not generally thread-safe.
            resource->Unbind(rb);
          }
        }
        resource->Release(can_make_gl_calls);
        resource->DeleteIfReleasedAndUnbound();
      }
      resources.clear();
    }
    gl_context_.Reset();
...
class Renderer::ResourceBinder : public Allocatable {
 public:
  ...
  explicit ResourceBinder(const GraphicsManagerPtr& gm)
      : graphics_manager_(gm),
        ...
        resources_to_unbind_(GetAllocator()) {
  ...
  void SetResourceManager(ResourceManager* manager) {
    // Need to have processed all unbinds before changing manager.
    DCHECK(resources_to_unbind_.empty() || manager == resource_manager_);
  ...
  void MarkForUnbind(ResourceManager::Resource* resource) {
    if (resource->GetResourceManager() != GetResourceManager())
      return;
    std::lock_guard<std::mutex> guard(resources_to_unbind_mutex_);
    resources_to_unbind_.push_back(resource);
    resource->IncrementUnbindRefCount();
  }

  void ProcessUnbinds() {
    std::lock_guard<std::mutex> guard(resources_to_unbind_mutex_);
    for (auto& resource : resources_to_unbind_) {
      DCHECK_EQ(resource->GetResourceManager(), GetResourceManager());
      resource->Unbind(this);
      resource->DecrementUnbindRefCount();
      resource->DeleteIfReleasedAndUnbound();
    }
    resources_to_unbind_.clear();
  }
 ...
 private:
  ...
  std::mutex resources_to_unbind_mutex_;
  base::AllocVector<ResourceManager::Resource*> resources_to_unbind_;

  friend class InfoRequestGuard;
};

void Renderer::ResourceManager::Resource::MarkForUnbindAll() {
  if (!IsValid())
    return;
  ...
  for (ResourceBinderMap::iterator it = binders.begin();
       ...
       ++it) {
    ResourceBinder* rb = it->second.get();
    rb->MarkForUnbind(this);
  }
  DeleteIfReleasedAndUnbound();
}

bool Renderer::ResourceManager::Resource::DeleteIfReleasedAndUnbound() {
  if (IsReleased() && GetUnbindRefCount() == 0) {
    delete this;
    return true;
  }
  return false;
...
void Renderer::ResourceManager::ReleaseResources(
    const HolderType* holder, ResourceBinder* binder) {
  typedef typename HolderToResource<HolderType>::ResourceType ResourceType;
  if (!holder)
    return;
  const std::vector<ResourceKey> keys =
  ...
  ProcessReleases();
  binder->ProcessUnbinds();
}

void Renderer::ResourceManager::ProcessReleases(bool can_make_gl_calls) {
...
class Renderer::Resource : public Renderer::ResourceManager::Resource {
 public:
  ...
  void OnDestroyed() override {
    ...
    // Note: Deleting ResourceHolder on another thread will be unsafe unless
    // we know no other threads are using GetHolder(), or GetHolder() does
    // a WeakReferentPtr::Acquire().
    DetachFromHolder();
...
class Renderer::SamplerResource : public Resource<Sampler::kNumChanges> {
 public:
  ...
  ~SamplerResource() override {
    DCHECK(IsReleased() || !portgfx::GlContext::GetCurrent());
  }
...
void Renderer::TextureResource::Unbind(ResourceBinder* rb) {
  ...
  if (rb) {
    ...
    if (IsReleased())
      rb->ClearAssignedImageUnit(this);
  }
}
...
void Renderer::ShaderProgramResource::UpdateUniformValuesAndImageUnits(
  ...
  for (size_t i = 0; i < uniform_count; ++i) {
    ...
    if (entry.sirr)
      entry.sirr->DetachShaderProgramResource(this, index_in_registry);
    entry.sirr = sirr;
...
void Renderer::TransformFeedbackResource::Unbind(ResourceBinder* rb) {
  if (rb)
    rb->ClearTransformFeedbackBinding(id_);
}
...
void Renderer::FramebufferResource::Unbind(ResourceBinder* rb) {
  // Note that we call ClearFramebufferBinding if resource_owns_gl_id_ is false.
  if (rb)
...
class Renderer::VertexArrayEmulatorResource
 ...
 public:
  ...
  void Unbind(ResourceBinder* rb) override;
  // As an emulator, there is no "valid" opengl id so always return true.
...
void Renderer::VertexArrayEmulatorResource::Unbind(ResourceBinder* rb) {
  if (rb && rb->GetActiveVertexArray() == this) {
    const bool can_make_gl_calls =
        rb->GetResourceManager()->AreResourcesAccessible();
    GraphicsManager* gm = rb->GetGraphicsManager().Get();
...
Renderer::~Renderer() {
  // Delete ShaderProgram here so that its ShaderProgramResources can be
  // deleted in DestroyOrAbandonAllResources().
...
Renderer::ResourceBinderMap& Renderer::GetResourceBinderMap() {
  // Ensure global resource count created before/destroyed after map so that
  // resouce counter exists for ~Resource() to decrement counter during
  // static deletion of ResourceBinders().
  GetGlobalResourceCount();
  ION_DECLARE_SAFE_STATIC_POINTER_WITH_CONSTRUCTOR(
      ResourceBinderMap, binders,
...
int Renderer::GetResourceCount() {
  return GetGlobalResourceCount();
}
...
void Renderer::BindFramebuffer(const FramebufferObjectPtr& fbo) {
  ...
  if (flags_.test(kProcessReleases))
    resource_manager_->ProcessReleases();
  resource_binder->ProcessUnbinds();
...
void Renderer::ResourceBinder::DrawScene(const NodePtr& node,
  ...
  if (flags.test(kProcessReleases))
    resource_manager_->ProcessReleases();
  ProcessUnbinds();
