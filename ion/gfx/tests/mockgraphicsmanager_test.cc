...
class MockGraphicsManagerTest : public ::testing::Test {
 public:
 ...
 protected:
  ...
  base::SharedPtr<MockVisual> visual_;
...
TEST_F(MockGraphicsManagerTest, Capabilities) {
...
TEST_F(MockGraphicsManagerTest, VersionStandardRenderer) {
  // Check defaults.
  EXPECT_EQ("3.3 Ion OpenGL / ES", gm_->GetGlVersionString());
  EXPECT_EQ(33U, gm_->GetGlVersion());
  EXPECT_EQ(GraphicsManager::kEs, gm_->GetGlApiStandard());

  gm_->SetVersionString("3.0 Ion OpenGL");
  EXPECT_EQ("3.0 Ion OpenGL", gm_->GetGlVersionString());
  EXPECT_EQ(30U, gm_->GetGlVersion());
  EXPECT_EQ(GraphicsManager::kDesktop, gm_->GetGlApiStandard());

  gm_->SetVersionString("1.2 Ion WebGL");
  EXPECT_EQ("1.2 Ion WebGL", gm_->GetGlVersionString());
  // WebGL is always 2.0 for compatibility with ES2.
  EXPECT_EQ(20U, gm_->GetGlVersion());
  EXPECT_EQ(GraphicsManager::kWeb, gm_->GetGlApiStandard());
...
TEST_F(MockGraphicsManagerTest, ProfileType) {
  // Non-desktop OpenGL platforms default to kCoreProfile.
  if (gm_->GetGlApiStandard() != GraphicsManager::kDesktop) {
...
TEST_F(MockGraphicsManagerTest, CallCount) {
  ...
  const int64 init_calls = MockGraphicsManager::GetCallCount();
  EXPECT_NE(0, init_calls);

  // GetEnabled calls IsEnabled and GetIntegerv once, and GetError twice, plus
  // the above calls.
  EXPECT_FALSE(GetEnabled(GL_BLEND));
  EXPECT_EQ(init_calls + 2, MockGraphicsManager::GetCallCount());

  GLuint ids[2];
  GM_CALL(GenTextures(2, ids));
  EXPECT_EQ(init_calls + 3, MockGraphicsManager::GetCallCount());

  EXPECT_FALSE(GetEnabled(GL_STENCIL_TEST));
  EXPECT_EQ(init_calls + 5, MockGraphicsManager::GetCallCount());
...
TEST_F(MockGraphicsManagerTest, InitialState) {
  GLboolean b4[4];
  GLfloat f4[4];
  GLint i4[4];
  ...
  EXPECT_EQ(GL_UNSIGNED_BYTE, GetInt(GL_IMPLEMENTATION_COLOR_READ_FORMAT));
  EXPECT_EQ(GL_RGB, GetInt(GL_IMPLEMENTATION_COLOR_READ_TYPE));
  EXPECT_EQ(1.f, GetFloat(GL_LINE_WIDTH));
  EXPECT_EQ(32, GetInt(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS));
  ...
  EXPECT_EQ(32.f, GetFloat(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS));
  EXPECT_EQ(16.f, GetFloat(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT));
  EXPECT_EQ(8192.f, GetFloat(GL_MAX_CUBE_MAP_TEXTURE_SIZE));
  EXPECT_EQ(512, GetInt(GL_MAX_FRAGMENT_UNIFORM_VECTORS));
  ...
  EXPECT_EQ(1024, GetInt(GL_MAX_VERTEX_UNIFORM_VECTORS));
...
TEST_F(MockGraphicsManagerTest, ChangeState) {
...
TEST_F(MockGraphicsManagerTest, BindTexture_ActiveTexture) {
  GLuint ids[5];
...
TEST_F(MockGraphicsManagerTest, TexParameter) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest, TexImage2D_GenerateMipmap) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest, TexImage3D) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest,
...
TEST_F(MockGraphicsManagerTest, TexSubImage3D_CopyTexSubImage3D) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(BindTexture(GL_TEXTURE_3D, ids[0]));
...
TEST_F(MockGraphicsManagerTest, CompressedTexImage2D_CompressedTexSubImage2D) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest, CompressedTexImage3D_CompressedTexSubImage3D) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest, TexImage2DMultisample) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest, TexImage3DMultisample) {
  GLuint ids[2];
  ids[0] = ids[1] = 0U;
  GM_CALL(GenTextures(2, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE4));
...
TEST_F(MockGraphicsManagerTest, GetMultisamplefv) {
...
TEST_F(MockGraphicsManagerTest, SampleMaski) {
  GLuint index = 3;
  GLbitfield mask = 19;
...
TEST_F(MockGraphicsManagerTest, TexStorage2D) {
  GLuint ids[3];
  memset(ids, 0, sizeof(ids));
...
TEST_F(MockGraphicsManagerTest, TexStorage3D) {
  GLuint ids[3];
  memset(ids, 0, sizeof(ids));
  GM_CALL(GenTextures(3, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE2));
...
TEST_F(MockGraphicsManagerTest, TexStorage2DMultisample) {
  GLuint ids[1];
  memset(ids, 0, sizeof(ids));
  GM_CALL(GenTextures(1, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE0));
...
TEST_F(MockGraphicsManagerTest, TexStorage3DMultisample) {
  GLuint ids[1];
  memset(ids, 0, sizeof(ids));
  GM_CALL(GenTextures(1, ids));
  GM_CALL(ActiveTexture(GL_TEXTURE0));
...
TEST_F(MockGraphicsManagerTest, Samplers) {
...
TEST_F(MockGraphicsManagerTest, ArraysBuffersDrawFunctions) {
  ...
  {
    portgfx::VisualPtr share_visual = MockVisual::CreateShared(*visual_);
...
TEST_F(MockGraphicsManagerTest, DrawInstancedFunctions) {
  // The default vertex buffer is 0.
  EXPECT_EQ(0, GetInt(GL_ARRAY_BUFFER_BINDING));
...
TEST_F(MockGraphicsManagerTest, MappedBuffers) {
  GLuint vbo = 0;
  GM_CALL(GenBuffers(1, &vbo));
  GM_CALL(BindBuffer(GL_ARRAY_BUFFER, vbo));
...
TEST_F(MockGraphicsManagerTest, FrameAndRenderBuffers) {
...
TEST_F(MockGraphicsManagerTest, FramebufferTextureLayerAttachments) {
...
TEST_F(MockGraphicsManagerTest, MultiviewAttachments) {
...
TEST_F(MockGraphicsManagerTest, MultisampleFramebuffers) {
  // GenFramebuffers.
  GLuint fb;
  GM_ERROR_CALL(GenFramebuffers(-1, &fb), GL_INVALID_VALUE);
  GM_CALL(GenFramebuffers(1, &fb));
...
TEST_F(MockGraphicsManagerTest, ResolveMultisampleFramebuffer) {
  const int width = 1024;
  const int height = 1024;
  const int samples = 8;
...
TEST_F(MockGraphicsManagerTest, IsExtensionSupportedParsesUnprefixedExtension) {
...
TEST_F(MockGraphicsManagerTest, FunctionGroupsAreDisabledByMissingExtensions) {
  // These tests are to increase coverage.
  EXPECT_TRUE(gm_->IsExtensionSupported("debug_label"));
  EXPECT_TRUE(gm_->IsExtensionSupported("vertex_array_object"));
  ...
  gm_->SetExtensionsString("GL_EXT_debug_label GL_OES_vertex_array_object");
  EXPECT_TRUE(gm_->IsExtensionSupported("debug_label"));
  EXPECT_TRUE(gm_->IsExtensionSupported("vertex_array_object"));
  EXPECT_TRUE(gm_->IsFeatureAvailable(GraphicsManager::kDebugLabel));
  EXPECT_TRUE(gm_->IsFeatureAvailable(GraphicsManager::kVertexArrays));
  gm_->SetExtensionsString("GL_OES_vertex_array_object");
  ...
  EXPECT_TRUE(gm_->IsExtensionSupported("debug_label"));
  EXPECT_FALSE(gm_->IsExtensionSupported("vertex_array_object"));
  EXPECT_TRUE(gm_->IsFeatureAvailable(GraphicsManager::kDebugLabel));
  EXPECT_TRUE(gm_->IsFeatureAvailable(GraphicsManager::kVertexArrays));
...
TEST_F(MockGraphicsManagerTest, GetString) {
...
TEST_F(MockGraphicsManagerTest, ProgramAndShaderFunctions) {
...
TEST_F(MockGraphicsManagerTest, Uniforms) {
  const std::string vertex_source(kVertexSource);
  const std::string geometry_source(kGeometrySource);
  const std::string fragment_source(kFragmentSource);
  GLuint vid = gm_->CreateShader(GL_VERTEX_SHADER);
...
TEST_F(MockGraphicsManagerTest, UniformArrays) {
  const std::string vertex_source(kVertexSource);
  const std::string fragment_source(kFragmentSource);
  GLuint vid = gm_->CreateShader(GL_VERTEX_SHADER);
  GLuint fid = gm_->CreateShader(GL_FRAGMENT_SHADER);
...
TEST_F(MockGraphicsManagerTest, ImageExternal) {
  // Just call the function.
  gm_->EGLImageTargetTexture2DOES(0, nullptr);
}

TEST_F(MockGraphicsManagerTest, ShaderPreprocessor) {
...
TEST_F(MockGraphicsManagerTest, ShaderPreprocessorUnsupportedFeatures) {
...
TEST_F(MockGraphicsManagerTest, PlatformCapabilities) {
  GLfloat f4[4];
  GLint i2[2];

  // Defaults.
  EXPECT_EQ(1.f, gm_->GetMinAliasedLineWidth());
  EXPECT_EQ(256.f, gm_->GetMaxAliasedLineWidth());
  EXPECT_EQ(1.f, gm_->GetMinAliasedPointSize());
  EXPECT_EQ(8192.f, gm_->GetMaxAliasedPointSize());
  EXPECT_EQ(static_cast<GLenum>(GL_UNSIGNED_BYTE),
            gm_->GetImplementationColorReadFormat());
  EXPECT_EQ(static_cast<GLenum>(GL_RGB), gm_->GetImplementationColorReadType());
  EXPECT_EQ(4096, gm_->GetMax3dTextureSize());
  EXPECT_EQ(4096, gm_->GetMaxArrayTextureLayers());
  EXPECT_EQ(32U, gm_->GetMaxCombinedTextureImageUnits());
  EXPECT_EQ(8192, gm_->GetMaxCubeMapTextureSize());
  EXPECT_EQ(256U, gm_->GetMaxFragmentUniformComponents());
  EXPECT_EQ(512U, gm_->GetMaxFragmentUniformVectors());
  ...
  EXPECT_EQ(512U, gm_->GetMaxVertexUniformComponents());
  EXPECT_EQ(1024U, gm_->GetMaxVertexUniformVectors());
  EXPECT_EQ(8192U, gm_->GetMaxViewportDims());

  // Set values and check that GL returns them.
  gm_->SetMinAliasedLineWidth(0.5f);
  EXPECT_EQ(0.5f, gm_->GetMinAliasedLineWidth());
  GM_CALL(GetFloatv(GL_ALIASED_LINE_WIDTH_RANGE, f4));
  EXPECT_EQ(0.5f, f4[0]);
...
TEST_F(MockGraphicsManagerTest, Tracing) {
  base::LogChecker log_checker;
...
TEST_F(MockGraphicsManagerTest, EnableAndDisableFunctionGroups) {
...
TEST_F(MockGraphicsManagerTest, ForceFailures) {
...
TEST_F(MockGraphicsManagerTest, DebugLabels) {
  char label[64];
  GLint length = 0;
...
TEST_F(MockGraphicsManagerTest, DebugMarkers) {
  base::LogChecker log_checker;
...
TEST_F(MockGraphicsManagerTest, DebugOutput) {
...
TEST_F(MockGraphicsManagerTest, DrawBuffer) {
  // Invalid enum.
...
TEST_F(MockGraphicsManagerTest, DrawBufferCompleteness) {
...
TEST_F(MockGraphicsManagerTest, ReadBuffer) {
  // Invalid enums.
...
TEST_F(MockGraphicsManagerTest, ReadBufferCompleteness) {
...
TEST_F(MockGraphicsManagerTest, MaxColorAttachmentsQuery) {
  EXPECT_EQ(4, gm_->GetCapabilityValue<int>(
  ...
  EXPECT_EQ(1, gm_->GetCapabilityValue<int>(
        GraphicsManager::kMaxColorAttachments));
}

TEST_F(MockGraphicsManagerTest, DrawBuffers) {
  ...
  GM_ERROR_CALL(
      DrawBuffers(
          gm_->GetCapabilityValue<int>(GraphicsManager::kMaxDrawBuffers) + 1,
          bufs1),
      GL_INVALID_VALUE);
...
TEST_F(MockGraphicsManagerTest, Sync) {
...
TEST_F(MockGraphicsManagerTest, DisjointTimerQuery) {
  GLuint ids[2] = {0, 0};
  GLint num = 0;
  GLuint unum = 0;
  GLint64 num64 = 0;
...
TEST_F(MockGraphicsManagerTest, TransformFeedbackFunctions) {
  GLuint ids[] = {1, 2};
...
TEST_F(MockGraphicsManagerTest, InvalidateFramebuffer) {
  // Invalid FB target.
...
TEST_F(MockGraphicsManagerTest, TiledRendering) {
  GLuint fb;
  GM_CALL(GenFramebuffers(1, &fb));
  GM_CALL(BindFramebuffer(GL_FRAMEBUFFER, fb));
