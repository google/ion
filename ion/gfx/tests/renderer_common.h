...
static void BuildRectangleBufferObject(Data* data, Options* options) {
  if (!data->vertex_buffer.Get())
    data->vertex_buffer = new BufferObject;
...
template <typename T>
static bool UploadThread(const RendererPtr& renderer, const VisualPtr& visual,
                         T* holder) {
...
bool UploadThread<ShapePtr>(const RendererPtr& renderer,
                            const VisualPtr& visual, ShapePtr* shapeptr) {
  // Set the Visual for this thread.
  portgfx::Visual::MakeCurrent(visual);
...
static bool RenderingThread(const RendererPtr& renderer,
                            const VisualPtr& visual, NodePtr* nodeptr) {
  portgfx::Visual::MakeCurrent(visual);
  renderer->DrawScene(*nodeptr);
  return true;
}

static bool UniformThread(const RendererPtr& renderer, const VisualPtr& visual,
