...
TEST_F(RendererTest, UnbindAndReleaseResource) {
  // Delete all ResourceBinders.
  Renderer::DestroyCurrentStateCache();

  int start_resource_count = Renderer::GetResourceCount();
  ...
  GlContextPtr gl_context_background =
      FakeGlContext::CreateShared(*gl_context_);
  portgfx::GlContext::MakeCurrent(gl_context_background);
  // Binds ShaderProgram and Textures in ResourceBinderA for
  // gl_context_background.
  EXPECT_EQ(0, renderer->GetResourceCount() - start_resource_count);
  renderer->DrawScene(root);
  // Resources are created as follows:
  //  Global ShaderInputRegistry A (context-specific)
  //  Graph ShaderInputRegistry A (context-specific)
  //  ShaderProgram
  //  Vertex Shader
  //  Geometry Shader
  //  Fragment Shader
  ...
  //  Texture
  //  CubeMapTexture
  //  VertexArray A (context-specific)
  //  BufferObject
  //  IndexBuffer
  EXPECT_EQ(12, renderer->GetResourceCount() - start_resource_count);

  // Binds ShaderProgram and Textures in ResourceBinderB for
  // gl_context_.
  portgfx::GlContext::MakeCurrent(gl_context_);
  renderer->DrawScene(root);
  // 3 extra resources created for 2 ShaderInputRegistrys and 1 VertexArray
  // since those types are per--ResourceBinder/context.
  // Global ShaderInputRegistry B (context-specific)
  // Graph ShaderInputRegistry B (context-specific)
  // VertexArray B (context-specific)
  EXPECT_EQ(15, renderer->GetResourceCount() - start_resource_count);

  ResourceManager* manager = renderer->GetResourceManager();
  CallbackHelper<ResourceManager::ProgramInfo> prog_info;
  manager->RequestAllResourceInfos<ShaderProgram, ResourceManager::ProgramInfo>(
      std::bind(&CallbackHelper<ResourceManager::ProgramInfo>::Callback,
                &prog_info, std::placeholders::_1));
  renderer->ProcessResourceInfoRequests();
  EXPECT_TRUE(prog_info.was_called);
  EXPECT_EQ(1U, prog_info.infos.size());

  CallbackHelper<ResourceManager::TextureInfo> tex_info;
  manager->RequestAllResourceInfos<TextureBase, ResourceManager::TextureInfo>(
      std::bind(&CallbackHelper<ResourceManager::TextureInfo>::Callback,
                &tex_info, std::placeholders::_1));
  renderer->ProcessResourceInfoRequests();
  EXPECT_TRUE(tex_info.was_called);
  // Texture used for uTexture, uTexture2 and CubeMapTexture.
  EXPECT_EQ(2U, tex_info.infos.size());

  // Delete all ResourceHolder, marking all resources for Release() and
  // Unbind().
  // Resources are marked for release as follows:
  //  IndexBuffer
  //  BufferObject
  //  VertexArray x 2
  //  CubeMapTexture
  ...
  //  Sampler
  //  Shader x 3
  //  ShaderInputRegistry x 2
  //  ShaderProgram
  DestroyGraph(data_, &root);

  // Expect that no resources were deleted.
  EXPECT_EQ(15, renderer->GetResourceCount() - start_resource_count);
  // Expect orphaned resources to have been removed from ResourceManager.
  prog_info.Reset();
  manager->RequestAllResourceInfos<ShaderProgram, ResourceManager::ProgramInfo>(
      std::bind(&CallbackHelper<ResourceManager::ProgramInfo>::Callback,
                &prog_info, std::placeholders::_1));
  renderer->ProcessResourceInfoRequests();
  EXPECT_TRUE(prog_info.was_called);
  EXPECT_EQ(0U, prog_info.infos.size());
  tex_info.Reset();
  manager->RequestAllResourceInfos<TextureBase, ResourceManager::TextureInfo>(
      std::bind(&CallbackHelper<ResourceManager::TextureInfo>::Callback,
                &tex_info, std::placeholders::_1));
  renderer->ProcessResourceInfoRequests();
  EXPECT_TRUE(tex_info.was_called);
  EXPECT_EQ(0U, tex_info.infos.size());

  // Release()es all resources and unbinds from ResourceBinderB only.
  Reset();
  portgfx::GlContext::MakeCurrent(gl_context_);
  renderer->DrawScene(NodePtr());
  // Expect Release() to call DeleteProgram, DeleteTextures.
  EXPECT_EQ(1U, trace_verifier_->GetCountOf("DeleteProgram"));
  EXPECT_EQ(2U, trace_verifier_->GetCountOf("DeleteTextures"));
  // Expect that resources have been removed from ResourceManager.
  prog_info.Reset();
  manager->RequestAllResourceInfos<ShaderProgram, ResourceManager::ProgramInfo>(
      std::bind(&CallbackHelper<ResourceManager::ProgramInfo>::Callback,
                &prog_info, std::placeholders::_1));
  renderer->ProcessResourceInfoRequests();
  EXPECT_TRUE(prog_info.was_called);
  EXPECT_TRUE(prog_info.infos.empty());
  tex_info.Reset();
  manager->RequestAllResourceInfos<TextureBase, ResourceManager::TextureInfo>(
      std::bind(&CallbackHelper<ResourceManager::TextureInfo>::Callback,
                &tex_info, std::placeholders::_1));
  renderer->ProcessResourceInfoRequests();
  EXPECT_TRUE(tex_info.was_called);
  EXPECT_TRUE(tex_info.infos.empty());
  // Expect that resources specific to ResourceBinderB have been deleted which
  // include Graph ShaderInputRegistryB and VertexArrayB. Note that the
  // resource for the global ShaderInputRegistry is leaked.
  EXPECT_EQ(13, renderer->GetResourceCount() - start_resource_count);

  // Unbind resources from ResourceBinderA which should delete them.
  portgfx::GlContext::MakeCurrent(gl_context_background);
  renderer->DrawScene(NodePtr());
  // Expect that resources have been deleted except for the global
  // ShaderInputRegistry resources which are leaked.
  EXPECT_EQ(2, renderer->GetResourceCount() - start_resource_count);
