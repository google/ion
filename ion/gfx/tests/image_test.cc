...
TEST(ImageTest, GetNumComponentsForFormat) {
  ...
  EXPECT_EQ(0, Image::GetNumComponentsForFormat(Image::kInvalid));
  {
    ...
    EXPECT_EQ(0, Image::GetNumComponentsForFormat(InvalidFormat()));
    base::RestoreDefaultBreakHandler();
#if ION_DEBUG
    EXPECT_TRUE(logchecker.HasMessage("DFATAL", "Unknown format"));
#endif
  }
}
...
TEST(ImageTest, ComputeDataSize) {
...

  {
    base::LogChecker logchecker;
    base::SetBreakHandler(kNullFunction);
    EXPECT_EQ(0U, Image::ComputeDataSize(InvalidFormat(), 20U, 16U));
    base::RestoreDefaultBreakHandler();
#if ION_DEBUG
    EXPECT_TRUE(logchecker.HasMessage("DFATAL", "Unknown format"));
#endif
