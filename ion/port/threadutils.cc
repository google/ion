...
// already set.
static std::thread::id InitMainThreadId(std::thread::id id) {
...
bool IsMainThread() {
  return std::this_thread::get_id() == InitMainThreadId(std::thread::id());
}

void SetMainThreadId(std::thread::id id) { InitMainThreadId(id); }
