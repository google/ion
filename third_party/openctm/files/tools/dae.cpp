...
Semantic ToSemantic(const std::string& semantic)
{
  if (semantic == "VERTEX")
    return VERTEX;
...
void ReadInputs(TiXmlElement* rootElem,bool& hasVerts,bool& hasNormals,bool& hasTexcoords, std::string& vertSource,std::string& normalSource,std::string& texcoordSource, vector<Input>& inputs) {
	...
	{
		if(std::string(inputElem->Value()) != "input")
			continue;
		//TiXmlHandle input(inputElem);
		inputs.push_back(Input());
		inputs.back().source = std::string(inputElem->Attribute("source")).substr(1);
...
Source& GetSource(map<std::string, Source >& sources, map<std::string, vector<Input> >& vertices,const std::string& source)
{
  map<std::string, Source >::iterator srcIterator = sources.find(source);
  if (srcIterator != sources.end())
    return srcIterator->second;
  map<std::string, vector<Input> >::iterator vertIterator = vertices.find(source);
...
void Import_DAE(TiXmlDocument &doc, Mesh * aMesh)
{
  ...
  for(elem = geometry.ToElement(); elem; elem=elem->NextSiblingElement())
  {
    ...
    if(meshElem)
    {
      ...
      {
        if(std::string(sourceElem->Value()) != "source")
          continue;
        TiXmlHandle source(sourceElem);
        std::string id = source.ToElement()->Attribute("id");
        TiXmlElement* arr = sourceElem->FirstChild("float_array")->ToElement();
        std::string str = arr->GetText();
    ...
    if (accessorElem->Attribute("offset"))
      ...
      map<std::string, vector<Input> > vertices;
      if (verticesElem) {
        std::string id = verticesElem->Attribute("id");
            ...
            inputElem; inputElem = inputElem->NextSiblingElement())
        {
          ...
          vertices[id].back().source = std::string(inputElem->Attribute("source")).substr(1);
      ...
      if(trianglesElem)
      {
        ...
        std::string vertSource = "", normalSource = "", texcoordSource = "";
    /*
        TiXmlElement* inputElem;
...
static std::string MakeISO8601DateTime(void)
{
  char buf[500];
#ifdef WIN32
  SYSTEMTIME tm;
...
/// Forward reference so that the file and stream loaders can use this function.
void Export_DAE(TiXmlDocument &doc, Mesh * aMesh, Options &aOptions);
...
void Export_DAE(const char * aFileName, Mesh * aMesh, Options &aOptions)
{
  ...
  if(xmlDoc.Error())
    throw runtime_error(std::string(xmlDoc.ErrorDesc()));
}

void Export_DAE(TiXmlDocument &xmlDoc, Mesh * aMesh, Options &aOptions) {
  TiXmlElement * elem;
  std::string dateTime = MakeISO8601DateTime();
