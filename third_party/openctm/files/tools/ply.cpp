...
static int PLYFaceCallback(p_ply_argument argument)
{
  ...
  if ((valueIndex < 0 ) || (valueIndex > 2))
    return 0;
  if (static_cast<size_t>(state->mFaceIdx) >=
      (state->mMesh->mIndices.size() / 3))
    return 0;
...
static int PLYVertexCallback(p_ply_argument argument)
{
  ...
  double value = ply_get_argument_value(argument);
  if (static_cast<size_t>(state->mVertexIdx) >= state->mMesh->mVertices.size())
    return 0;
  switch(index)
  {
    case 0:
...
static int PLYNormalCallback(p_ply_argument argument)
{
  ...
  ply_get_argument_user_data(argument, (void **) &state, &index);
  double value = ply_get_argument_value(argument);
  if (static_cast<size_t>(state->mNormalIdx) >= state->mMesh->mNormals.size())
    return 0;
  switch(index)
  {
    case 0:
...
static int PLYTexCoordCallback(p_ply_argument argument)
{
  ...
  ply_get_argument_user_data(argument, (void **) &state, &index);
  double value = ply_get_argument_value(argument);
  if (static_cast<size_t>(state->mTexCoordIdx) >= state->mMesh->mTexCoords.size())
    return 0;
  switch(index)
  {
    case 0:
...
static int PLYColorCallback(p_ply_argument argument)
{
  ...
  ply_get_argument_user_data(argument, (void **) &state, &index);
  double value = ply_get_argument_value(argument);
  if (static_cast<size_t>(state->mColorIdx) >= state->mMesh->mColors.size())
    return 0;
  switch(index)
  {
    case 0:
...
void Import_PLY(const char * aFileName, Mesh * aMesh)
{
  ...
  {
    if(firstComment)
      aMesh->mComment = std::string(comment);
    else
      aMesh->mComment += std::string(" ") + std::string(comment);
    firstComment = false;
