...
static std::string ReadString(istream &aStream, int aCount)
{
  std::string result;
...
static std::string ReadStringZ(istream &aStream)
{
  std::string str;
...
void Import_LWO(std::istream &f, Mesh * aMesh)
{
  ...
  while(!f.eof() && (f.tellg() < fileSize))
  {
    ...
    // are word aligned)
    std::string chunkID = ReadString(f, 4);
    ...
    if(chunkID == std::string("TEXT"))
    {
      // Read file comment
      aMesh->mComment = std::string(ReadStringZ(f));
    }
    else if(chunkID == std::string("LAYR"))
    {
    ...
    else if(chunkID == std::string("PNTS"))
    {
      // Check point count
    ...
    else if(chunkID == std::string("POLS"))
    {
      // POLS before PNTS?
      if(!havePoints)
      ...
      std::string type = ReadString(f, 4);
      if((type == std::string("FACE")) || (type == std::string("PTCH")))
      {
    ...
    else if((chunkID == std::string("VMAP")) || (chunkID == std::string("VMAD")))
    {
      bool dynamic = (chunkID == std::string("VMAD"));
      std::string type = ReadString(f, 4);
      ...
      if((type == std::string("RGB ")) || (type == std::string("RGBA")))
      {
