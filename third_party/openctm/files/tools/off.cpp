...
static void ReadNextLine(istream &aStream, std::string &aResult, std::string &aComment)
{
  while(true)
  {
    ...
    {
      std::string lineComment = TrimString(line.substr(commentPos + 1));
      if(lineComment.size() > 0)
      {
        if(aComment.size() > 0)
          aComment = aComment + std::string(" ") + lineComment;
        else
...
static void ParseVeretex(const std::string aString, Vector3 * aCoord, Vector4 * aColor)
{
