...
class VRMLReader {
  private:
    ...
    void ReadComment()
    {
      ...
      std::string newComment = TrimString(GetNextLine());

      // Append the line comment to the global comment string
      if(newComment.size() > 0)
      {
        if(mComment.size() > 0)
          mComment = mComment + std::string(" ");
  ...
  public:
    ...
    void ReadMesh(Mesh * aMesh)
    {
      ...
      if(header.substr(0, 10) != std::string("#VRML V2.0"))
      ...
      while(!mEndOfFile)
      {
        ...
        if(token == std::string("geometry"))
        {
          token = GetNextToken();
          if(token == std::string("IndexedFaceSet"))
          {
            // ...FIXME
          }
        }
