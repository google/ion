...
typedef enum {
  ...
  CTM_NONE = 0x0000,  ///< No error has occured (everything is OK).
                      ...
                      ///  value.
  CTM_INVALID_CONTEXT =
      0x0001,  ///< The OpenCTM context was invalid (e.g. NULL).
  ...
  CTM_BAD_FORMAT = 0x0007,  ///< File format error (e.g. unrecognized format or
  /// corrupted file).
  CTM_LZMA_ERROR = 0x0008,  ///< An error occured within the LZMA library.
  CTM_INTERNAL_ERROR =
      0x0009,  ///< An internal error occured (indicates a bug).
  CTM_UNSUPPORTED_FORMAT_VERSION =
      0x000A,  ///< Unsupported file format version.
  CTM_LIMIT_VIOLATION =
      0x000B,  ///< A limit contraint was violated while deserializing.

  // OpenCTM context modes
  CTM_IMPORT =
      0x0101,  ///< The OpenCTM context will be used for importing data.
  CTM_EXPORT =
      0x0102,  ///< The OpenCTM context will be used for exporting data.
  ...
  CTM_ATTRIB_MAP_COUNT =
      0x0305,  ///< Number of custom attribute sets (integer).
  ...
  // Lossless compression provider.
  CTM_COMPRESSION_COMPATIBLE =
      0x9001,                    ///< Default. Use openctm's copy of liblzma.
  CTM_COMPRESSION_ZLIB = 0x9002  ///< Use zlib (much faster, but incompatible).
} CTMenum;

// Google-authored modification:
CTMEXPORT void CTMCALL ctmSetLimits(CTMcontext aContext, CTMlimits limits);
// End Google-authored modification.
...
/// Set which compression provider to use for the given OpenCTM context.
/// The selected compression provider will be used when calling the ctmSave()
/// function.
/// @param[in] aContext An OpenCTM context that has been created by
///            ctmNewContext().
/// @param[in] aProvider Which compression method to use:
///            CTM_COMPRESSION_COMPATIBLE or CTM_COMPRESSION_ZLIB (the default
///            provider is CTM_COMPRESSION_COMPATIBLE).
/// @see CTM_COMPRESSION_COMPATIBLE, CTM_COMPRESSION_ZLIB
CTMEXPORT void CTMCALL ctmCompressionProvider(CTMcontext aContext,
  CTMenum aProvider);

/// Set which compression level to use for the given OpenCTM context.
