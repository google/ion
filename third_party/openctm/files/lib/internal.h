...
typedef struct {
  ...
  // The selected lossless comrpession provider
  CTMenum mCompressionProvider;
  ...
  // Google-authored addition for hardening.
  CTMlimits *mLimits;
} _CTMcontext;
...
//-----------------------------------------------------------------------------
// Google-authored hardening
//-----------------------------------------------------------------------------

size_t _ctmStringReadLimit(_CTMcontext *self);

// This is a malloc that respects the maxAllocation limit setting.
CTMEXPORT void *CTMCALL _ctmSafeMalloc(_CTMcontext *self, size_t size);

// This is a special-purpose malloc that protects against unsigned overflow.
// Otherwise, when you want N of size-K things, and N*K overflows, you get less
// than you need. Respects maxAllocation setting.
void *_ctmSafeMallocArray(_CTMcontext *self, size_t blockCount,
...
int _ctmStreamReadSTRING(_CTMcontext *self, char **aValue,
                         size_t maxStringLength);
void _ctmStreamWriteSTRING(_CTMcontext *self, const char *aValue);
int _ctmStreamReadPackedInts(_CTMcontext *self, CTMint *aData, CTMuint aCount,
                             CTMuint aSize, CTMint aSignedInts);
int _ctmStreamWritePackedInts(_CTMcontext *self, CTMint *aData, CTMuint aCount,
                              CTMuint aSize, CTMint aSignedInts);
...
#endif
// Visible for testing.
int _compress(_CTMcontext *self, unsigned char *in, size_t in_size,
              unsigned char *out, size_t *out_size, unsigned char *outprops,
              size_t *outprops_size);

int _uncompress(unsigned char *in, size_t in_size, unsigned char *out,
                size_t *out_size, unsigned char *props, size_t props_size);
#ifdef __cplusplus
}
#endif
